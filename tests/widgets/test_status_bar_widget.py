...
def test_status_bar_has_five_fields(qtbot):
    bar = StatusBarWidget()
    qtbot.addWidget(bar)

    assert len(bar._fields) == 5
...
def test_set_fields_updates_text_and_accessible_name(qtbot):
    ...
    bar.set_fields([
        ...
        "Playback tempo: 120 quarter notes per minute (score default)", "Playback: Playing",
    ])
...
def test_set_field_updates_only_the_given_field(qtbot):
    bar = StatusBarWidget()
    qtbot.addWidget(bar)
    bar.set_fields(["Measure 1 beat 1", "Key: -", "Time: 4/4", "Playback tempo: -", "Playback: Stopped"])
...
def test_tab_moves_to_the_next_field_and_wraps(qtbot):
    ...
    calls.clear()
    qtbot.keyClick(bar._fields[4], Qt.Key.Key_Tab)
...
def test_shift_tab_moves_to_the_previous_field_and_wraps(qtbot):
    ...
    assert calls == [4], "wraps from the first field back to the last"
